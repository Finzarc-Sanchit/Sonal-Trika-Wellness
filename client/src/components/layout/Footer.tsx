...
function FooterNewsletter() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      await newsletterService.subscribe({ email: trimmedEmail });
      setError('');
      setSubmitted(true);
      pushToast('Subscribed', "Thank you — you’re on the list.");
    } catch {
      pushToast(
        'Unable to subscribe',
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-3">
            <div className="flex min-w-0 flex-col gap-3 lg:flex-row lg:items-center">
              ...
              <input
                ...
                onBlur={() => {
                  setTouched(true);
                  if (!submitted) setError(emailError);
                }}
              ...
              <motion.button
                ...
                className={`inline-flex w-full shrink-0 items-center justify-center rounded-full bg-[#A55A42] px-5 py-2.5 font-sans text-[11px] font-semibold uppercase tracking-[0.16em] text-white transition-colors duration-400 border-0 lg:w-auto ${
                  !isFormValid || isSubmitting ? 'opacity-70 cursor-not-allowed' : 'hover:bg-[#8e4a35] cursor-pointer'
                }`}
              >
                {isSubmitting ? 'Subscribing…' : 'Subscribe'}
              </motion.button>
            </div>
            {((touched || submitAttempted) && error) ? (
              <p className="font-sans text-caption text-red-500">{error}</p>
            ) : null}
          </form>
        </>
      )}
